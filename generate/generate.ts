...
function genFeatureExport(features: any, feature: string, fw: any) {
  ...
  switch (feature) {
    case "cpu":
      ...
      cpu +=
        features[key].cores === 1
          ? "1"
          : "{x}`, `x=[1," + features[key].cores + "]";
      cpu += "`</li>";
      return cpu;
    case "network":
      let interfaces = "interfaces:";
      for (const interf in defaultInterfaces)
        interfaces += "<li>`" + defaultInterfaces[interf] + "`</li>";
      for (const interf in features.wlan)
        interfaces +=
          "<li>`" +
          labelInterfaces[interf as keyof object] +
          "`" +
          (interf === "6ghz" ? "[^6ghz]" : "") +
          "</li>";
      return interfaces;
        ...
        for (const port in features[key][type]) {
          if (features[key][type][port].id instanceof Array) {
            ports +=
              ...
              type +
              "_{x}`, `x=[" +
              features[key][type][port].id[0] +
              "," +
              features[key][type][port].id[1] +
              "]`</li><li>up to `" +
              features[key][type][port].speed +
              " Mb/s`</li>";
          } else
            ports +=
              "<li>`" +
    ...
    case "temperature":
      let temperature = "";
      temperature +=
        fw[386] === true || fw[388] === true || fw[106] === true
          ? "Stock:<li>`CPU`</li>Merlin:<li>`CPU`</li>"
          : "Merlin:";
      for (const interf in features.wlan)
        temperature +=
          "<li>`" + labelInterfaces[interf as keyof object] + "`</li>";
      return temperature;
    case "wlan":
      let wlan = "types:";
      for (const interf in features.wlan)
        wlan += "<li>`" + labelInterfaces[interf as keyof object] + "`</li>";
      return wlan;
    default:
      return "`default`";
  }
...
function genFeatureSpecs(features: any, feature: string) {
  ...
  switch (feature) {
    case "cpu":
      let cpu = "";
      if (features[key].frequency !== undefined)
        cpu += features[key].frequency + " ";
      cpu += labelCPU[features[key].cores as keyof object] + " processor";
      if (features[key].model != undefined)
        cpu += "<li>`" + features[key].model + "`</li>";
      return cpu;
    case "guest-wlan":
      let guest_wlan = "";
      for (const interf in features.wlan)
        guest_wlan +=
          "<li>3x `" + labelInterfaces[interf as keyof object] + "`</li>";
      return guest_wlan;
    case "ports":
      let ports = "";
...
function genFeatureNotes(features: any) {
  let notes = "";
  if (features.wlan["6ghz"] !== undefined)
    notes +=
...
export async function generateDevice(device: any) {
  ...
  let content = `---
title: "Asus ${title}"
description: "Integrate your ${
    device.model
  } into Home Assistant with AsusRouter"
---
<!-- !!! -->
...
|---|---|
|<img src="${await getImage(model)}" width="300">|<b>Stock:</b>${genFWversions(
    device.tested.stock
  )}<b>Merlin:</b>${genFWversions(device.tested.merlin)}|
|${
    device.links.asus !== undefined
      ? '<li><a href="' +
        device.links.asus +
        '" rel="nofollow" target="_blank">Official webpage</a></li>'
      : ""
  }${
    device.links.amazon !== undefined
      ? '<li><a href="' +
        device.links.amazon +
        '" rel="nofollow sponsored" target="_blank">Buy it on Amazon [^amazon]</a></li>'
      : ""
  }|${
    device.tested.state === true
      ? "<li>:green_heart: Tested to work with AsusRouter</li>"
      : "<li>:yellow_heart: This device has not been tested yet, but should work with AsusRouter</li>"
  }${
    device.tested.available ? "<li>:green_heart: Available for tests</li>" : ""
  }<li><a href="https://github.com/Vaskivskyi/ha-asusrouter/issues/new/choose" rel="nofollow" target="_blank">Add you report about this device</a></li>|
...
export async function generateDeviceList(devices: any[]) {
  ...
  return generatePage(
    JSON.stringify(list),
    path.resolve(deviceBaseDir, "map.json")
  );
}
...
`;
    ...
    sorted[key].forEach(function (device) {
      ...
      if (device.tested.stock.length !== 0) {
        content += "Stock:";
        for (const version in device.tested.stock)
          content += "<li>`" + device.tested.stock[version] + "`</li>";
      }
      if (device.tested.merlin.length !== 0) {
        content += "Merlin:";
        for (const version in device.tested.merlin)
          content += "<li>`" + device.tested.merlin[version] + "`</li>";
      }
      content += "|";
      if (device.links.amazon !== undefined)
        content +=
          '<a href="' +
          device.links.amazon +
          '" rel="nofollow sponsored" target="_blank">find it</a>';
      content += "|\n";
    });
  }
  content +=
